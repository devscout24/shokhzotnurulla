...
@push('page-styles')
    <style>
        ...
        .bulk-table-header {
            display: grid;
            grid-template-columns: 160px 150px 180px 180px 200px 140px 180px 300px 110px 40px;
            gap: 12px;
            padding: 12px 15px;
            background: #fff;
            border-bottom: 1px solid #eee;
        ...
        .bulk-col-h1 { width: 180px; }
        .bulk-col-slug { width: 180px; }
        .bulk-col-mt { width: 180px; }
        .bulk-col-md { width: 220px; }
        .bulk-col-p { width: 120px; }
        .bulk-col-cat { width: 150px; }
        .bulk-col-c { width: 500px; }
        .bulk-col-s { width: 120px; }
        .bulk-col-a { width: 50px; }
    </style>
@endpush
        ...
        <div style="padding: 30px 45px 0 45px;">
            <div class="page-header" style="margin-bottom: 20px; border: none;">
                <h2 class="view-title" style="font-size: 24px; font-weight: 700; color: #222;">
                    {{ __('Reusable Content: Static Page Content') }}</h2>
                <div class="rc-header-actions">
                    ...
                    <button class="rc-btn-outline" type="button" id="bulkEditBtn"><i class="bi bi-pencil-square"></i>
                        {{ __('Bulk Edit') }}</button>
                    <button class="rc-btn-add" type="button" id="addSpcBtn"><i class="bi bi-plus-lg"></i>
                        {{ __('Add New Static Page Content') }}</button>
                </div>
            </div>
                ...
                <div style="flex: 1; display: flex; flex-direction: column;">
                    ...
                    <div id="spcFormView" style="display: none;">
                        <div style="margin-bottom: 20px;">
                            <button class="rc-btn-outline" id="spcBackBtn" style="height: 38px;"><i
                        ...
                        <div class="rc-main-container" style="padding: 0;">
                            <div style="padding: 20px 30px; border-bottom: 1px solid #f0f0f0;">
                                <h3 style="font-size: 15px; font-weight: 600; color: #333; margin: 0;" id="formViewTitle">
                                    Add Static Page Content</h3>
                            </div>
                            <div style="padding: 30px;">
                                <div style="margin-bottom: 25px;">
                                    <label class="bulk-col-label">H1 Override <span>*</span></label>
                                    <input type="text" id="spcH1Input" class="bulk-input"
                                        placeholder="Main header of the page">
                                </div>

                                <div style="margin-bottom: 25px;">
                                    <label class="bulk-col-label">Meta Title / Title Tag Override (optional)</label>
                                    <input type="text" id="spcMetaTitleInput" class="bulk-input"
                                        placeholder="Meta title">
                                </div>

                                <div style="margin-bottom: 25px;">
                                    <label class="bulk-col-label">Meta Description (optional override - ideally 160
                                        characters or less)</label>
                                ...
                                <div style="margin-bottom: 25px;" class="slug-input-wrap">
                                    ...
                                    <div class="field-hint">
                                        Exact match: <strong>/incentives</strong> or
                                        <strong>/incentives?make[]=Chevrolet</strong>. Add '*' for wildcard:
                                        <strong>/incentives*</strong> matches any params. Use | for OR:
                                        <strong>/incentives?make[]=Chrysler|Dodge|Jeep|Ram</strong> matches any of those
                                        makes.
                                    </div>
                                </div>
                                ...
                                <div style="margin-bottom: 30px;">
                                    ...
                                    <div class="bulk-rte">
                                        ...
                                        <div contenteditable="true" id="spcContentInput" class="rte-textarea"></div>
                                    </div>
                                </div>

                                <div style="display: flex; justify-content: flex-start;">
                                    <button class="btn-save-red" id="spcSaveBtn"><i class="bi bi-check-lg"></i>
                                        Save</button>
                                </div>
                            </div>
                        </div>
    ...
    <div class="faq-modal-overlay" id="catModalOverlay">
        <div class="faq-modal">
            <div class="faq-modal-header"
                style="display: flex; justify-content: space-between; align-items: center; padding: 20px 25px; border-bottom: 1px solid #eee;">
                <h3 class="faq-modal-title" id="catModalTitle" style="margin: 0; font-size: 18px; font-weight: 700;">
                    Manage Categories</h3>
                <button class="faq-modal-close" id="catModalClose"
                    style="background: none; border: none; font-size: 24px; color: #aaa; cursor: pointer;">&times;</button>
            </div>
        ...
        <div class="faq-modal">
            <div class="bulk-modal-content">
                <div class="bulk-header-top">
                    <h3 class="faq-modal-title" style="font-size: 14px; font-weight: 600;">Bulk Edit Content: Static Page
                        Content</h3>
                    <div style="display: flex; align-items: center; gap: 15px;">
                        <button class="rc-btn-outline" id="bulkAddRowBtn"
                            style="padding: 7px 15px; background: #fff; height: 34px;"><i class="bi bi-plus-lg"></i> Add
                            Row</button>
                ...
                <div class="bulk-body">
                    <div class="bulk-table-header">
                        <div class="bulk-header-label bulk-col-h1">H1 Override <span>*</span></div>
                        <div class="bulk-header-label bulk-col-slug">Slug <span>*</span></div>
                        <div class="bulk-header-label bulk-col-mt">Meta Title</div>
                        <div class="bulk-header-label bulk-col-md">Meta Description</div>
                        <div class="bulk-header-label bulk-col-p">Placement</div>
                        <div class="bulk-header-label bulk-col-cat">Category</div>
                        <div class="bulk-header-label bulk-col-c">Page Content</div>
                        <div class="bulk-header-label bulk-col-s">Status</div>
                        <div class="bulk-header-label bulk-col-a"></div>
                    </div>
...
@push('page-scripts')
    <script>
        (function() {
            ...
            function renderTable() {
                ...
                contents.forEach(function(c) {
                    ...
                    html += '<tr>' +
                        ...
                        '<a href="#" class="rc-row-btn" onclick="window._spcEdit(' + c.id +
                        ');return false;"><i class="bi bi-pencil-square"></i> Edit</a>' +
                        '<button class="rc-row-btn trash-btn" onclick="window._spcTrash(' + c.id +
                        ...
                        '<td><span style="color:#666; font-size:14px;">' + (c.author || 'System') +
                        '</span></td>' +
                        '<td>' +
            ...
            document.getElementById('addSpcBtn').addEventListener('click', function() {
                openForm(null);
            });
            ...
            function openForm(id) {
                editingId = id;
                var item = id ? contents.find(function(c) {
                    return c.id === id;
                }) : null;
                document.getElementById('formViewTitle').textContent = item ? 'Edit Static Page Content' :
            ...
            document.getElementById('spcSaveBtn').addEventListener('click', function() {
                var payload = {
                    ...
                    content: document.getElementById('spcContentInput').innerHTML,
                    status: 'Published'
                };
                ...
                if (editingId) {
                    ajax('PATCH', ROUTES.update.replace('__ID__', editingId), payload, function(err, res) {
                        ...
                        var idx = contents.findIndex(function(c) {
            ...
            function showCatEdit(cat) {
                ...
                body.innerHTML = '<div style="padding:25px;"><label class="bulk-col-label">Category Name</label>' +
                    ...
                    (isNew ? '' :
                        '<button class="rc-row-btn trash-btn" id="catDeleteBtn" style="margin-left:auto; padding: 9px 15px;">Delete</button>'
                        ) +
                    '</div></div>';
            ...
            function addBulkRow(item) {
                ...
                categories.forEach(function(cat) {
                    catOpts += '<option value="' + cat.id + '"' + (item && item.static_page_category_id == cat
                        .id ? ' selected' : '') + '>' + cat.name + '</option>';
                });
                ...
                div.innerHTML =
                    '<input type="text" class="bulk-input bulk-h1 bulk-col-h1" value="' + (item ? (item.h1_override || '') : '') +
                    '" placeholder="H1">' +
                    '<input type="text" class="bulk-input bulk-s bulk-col-slug" value="' + (item ? item.slug : '') +
                    '" placeholder="/slug">' +
                    '<input type="text" class="bulk-input bulk-mt bulk-col-mt" value="' + (item ? (item.meta_title || '') : '') +
                    '" placeholder="Meta Title">' +
                    '<textarea class="bulk-input bulk-md bulk-col-md" style="height:120px;" placeholder="Meta Desc">' + (item ? (item
                        .meta_description || '') : '') + '</textarea>' +
                    '<select class="bulk-select bulk-p bulk-col-p">' +
                    '<option value="top" ' + (item && item.placement === 'top' ? 'selected' : '') + '>Top</option>' +
                    '<option value="bottom" ' + (item && item.placement === 'bottom' ? 'selected' : '') +
                    '>Bottom</option>' +
                    '</select>' +
                    '<select class="bulk-select bulk-cat bulk-col-cat">' + catOpts + '</select>' +
                    '<div class="bulk-rte-wrap bulk-col-c">' + toolbarHtml + 
                    '<div contenteditable="true" class="bulk-rte-content bulk-c">' + (item ? (item.content || '') : '') + '</div></div>' +
                    '<select class="bulk-select bulk-status bulk-col-s">' +
                    ...
                    '</select>' +
                    '<div class="bulk-action-del bulk-col-a"><i class="bi bi-trash"></i></div>';
