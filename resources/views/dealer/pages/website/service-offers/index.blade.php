...
@section('title', __('Reusable Content: Offer') . ' | ' . __(config('app.name')))
...
@push('page-styles')
    ...
    <style>
        ...
        /* Form Styles */
        .bulk-col-label { font-size: 11px; font-weight: 700; color: #444; margin-bottom: 6px; display: block; text-transform: none; }
        .bulk-col-label span { color: #d0021b; margin-left: 2px; }
        .bulk-input, .bulk-select { width: 100%; padding: 8px 12px; border: 1px solid #eef0f2; border-radius: 4px; font-size: 13px; color: #333; transition: all .2s; background: #fff; height: 38px; }
        .bulk-input:focus, .bulk-select:focus { border-color: #d0021b; outline: none; background: #fff; box-shadow: none; }
        ...
        .btn-save-red { background: #d0021b; color: #fff; border: none; padding: 8px 25px; border-radius: 4px; font-weight: 700; font-size: 13px; cursor: pointer; display: flex; align-items: center; gap: 8px; transition: background .2s; }
        ...
        .bulk-modal { background: #f0f2f5; width: 98%; max-width: 1700px; height: 95vh; border-radius: 12px; display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 20px 60px rgba(0,0,0,0.3); }
        ...
        .bulk-table { width: 100%; border-collapse: collapse; min-width: 1600px; }
        ...
        .bulk-rte-wrap { width: 450px; border: 1px solid #eef0f2; border-radius: 6px; overflow: hidden; background: #fff; }
        ...
        .photo-preview-box { width: 100%; max-width: 400px; margin-top: 15px; border: 1px solid #eee; border-radius: 8px; overflow: hidden; background: #f8f9fa; display: none; }
        .photo-preview-box img { width: 100%; display: block; }
        .btn-select-media { border: none; background: none; padding: 0; font-size: 11px; cursor: pointer; display: flex; align-items: center; gap: 4px; color: #d0021b; font-weight: 600; text-decoration: none; }
        .btn-select-media:hover { text-decoration: underline; }
...
<main class="main-content" id="mainContent" style="padding:0; background: #f0f2f5; min-height: 100vh;">
    <div style="padding: 30px 45px;">
        <div class="page-header" style="margin-bottom: 25px; border: none; background: transparent; padding: 0;">
            <h2 class="view-title" style="font-size: 24px; font-weight: 700; color: #222;">{{ __('Reusable Content: Offer') }}</h2>
            <div class="rc-header-actions">
                ...
                <button class="rc-btn-add" type="button" id="addBtn"><i class="bi bi-plus-lg"></i> {{ __('Add New Offer') }}</button>
            </div>
        </div>
            ...
            <div style="flex: 1; display: flex; flex-direction: column;">
                ...
                <div id="listView">
                    <div class="rc-main-container">
                        <table class="rc-table" id="offerTable">
                            <thead>
                                <tr>
                ...
                <div id="formView" style="display: none;">
                    ...
                    <div class="rc-main-container" style="border: 1px solid #e0e0e0; border-radius: 8px;">
                        <div style="padding: 15px 30px; border-bottom: 1px solid #f0f0f0; display: flex; justify-content: space-between; align-items: center; background: #fff;">
                            <h3 style="font-size: 14px; font-weight: 700; color: #333; margin: 0;" id="formViewTitle">Add Offer</h3>
                        </div>
                        <div style="padding: 30px; background: #fff;">
                            
                            <div style="margin-bottom: 25px;">
                                <label class="bulk-col-label">Title</label>
                                <input type="text" id="titleInput" class="bulk-input" placeholder="Spring cleaning special">
                            </div>

                            <div style="margin-bottom: 25px;">
                                <label class="bulk-col-label">Subtitle</label>
                                <input type="text" id="subtitleInput" class="bulk-input" placeholder="Price or percentage discount offered">
                            </div>
                            ...
                            <div style="margin-bottom: 25px;">
                                ...
                                <div class="bulk-rte">
                                    ...
                                    <div contenteditable="true" id="descInput" class="rte-textarea" style="background: #fcfdfe; border-top: 1px solid #eef0f2;"></div>
                                </div>
                            </div>
                            ...
                            <div style="margin-bottom: 25px;">
                                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                                    ...
                                    <button class="btn-select-media" type="button" onclick="window.openMediaSelector('photoInput', 'photoPreview')"><i class="bi bi-images"></i> Select Media</button>
                                </div>
                                <div class="photo-field-wrap">
                                    <input type="text" id="photoInput" class="bulk-input" placeholder="https://path.to.image/photo.jpg">
                            ...
                            <div style="margin-bottom: 25px;">
                                <label class="bulk-col-label">Link offer to <span>*</span></label>
                                <input type="text" id="linkToInput" class="bulk-input" placeholder="/schedule-service">
                            </div>

                            <div style="margin-bottom: 25px;">
                                <label class="bulk-col-label">Link text <span>*</span></label>
                                <input type="text" id="linkTextInput" class="bulk-input" placeholder="Schedule Service">
                            </div>
                            ...
                            <div style="margin-bottom: 35px;">
                                ...
                                <div class="bulk-rte">
                                    ...
                                    <div contenteditable="true" id="disclaimerInput" class="rte-textarea" style="background: #fcfdfe; border-top: 1px solid #eef0f2;"></div>
                                </div>
                            </div>
...
<div class="bulk-overlay" id="bulkOverlay">
    <div class="bulk-modal">
        <div class="bulk-header">
            <h2 style="font-size: 18px; font-weight: 700; margin: 0; color: #333;">Bulk Edit Service Offers</h2>
...
(function(){
    ...
    var ROUTES = {
        store: "{{ route('dealer.website.service-offers.store') }}",
        update: "{{ route('dealer.website.service-offers.update', '__ID__') }}",
        destroy: "{{ route('dealer.website.service-offers.destroy', '__ID__') }}",
        bulkUpdate: "{{ route('dealer.website.service-offers.bulk-update') }}",
        catStore: "{{ route('dealer.website.service-offers.categories.store') }}",
        catUpdate: "{{ route('dealer.website.service-offers.categories.update', '__ID__') }}",
        catDestroy: "{{ route('dealer.website.service-offers.categories.destroy', '__ID__') }}"
    };
    ...
    window._trash = function(id){
        customConfirm('Are you sure you want to delete this offer?', function(){
            ajax('DELETE', ROUTES.destroy.replace('__ID__', id), null, function(err){
                if(err) return alert(err);
                offers = offers.filter(r => r.id != id); renderTable(); showToaster('Offer deleted.');
            });
        });
    };
    ...
    function renderTable(){
        ...
        var html = '';
        var filtered = offers.filter(b => (catFilter === 'All' || b.service_offer_category_id == catFilter));
    ...
    window.openForm = function(id){
        editingId = id;
        var item = id ? offers.find(function(b){ return b.id == id; }) : null;
        document.getElementById('formViewTitle').textContent = item ? 'Edit Offer' : 'Add Offer';
        ...
        document.getElementById('linkToInput').value = item ? item.link_offer_to : '';
        ...
        categories.forEach(function(c){ catSel.innerHTML += '<option value="'+c.id+'"'+(item && item.service_offer_category_id == c.id ? ' selected' : '')+'>'+c.name+'</option>'; });
    ...
    document.getElementById('photoInput').oninput = function(){ updatePhotoPreview('photoInput', 'photoPreview'); };
    ...
    document.getElementById('saveBtn').onclick = function(){
        var p = {
            ...
            service_offer_category_id: document.getElementById('categorySelect').value,
            photo_url: document.getElementById('photoInput').value.trim(),
            link_offer_to: document.getElementById('linkToInput').value.trim(),
        ...
        if(!p.service_offer_category_id) return alert('Category is required');
        if(!p.link_offer_to) return alert('Link Offer To is required');
        ...
        ajax(editingId?'PATCH':'POST', editingId?ROUTES.update.replace('__ID__', editingId):ROUTES.store, p, function(err, res){
            if(err) return alert(err);
            if(editingId){ var idx = offers.findIndex(function(b){ return b.id == editingId; }); if(idx>-1) offers[idx] = res; }
            else offers.push(res);
            window.location.hash = '#list'; renderTable(); showToaster('Offer saved.');
        });
    };
    ...
    function openBulkEdit(){
        ...
        if(offers.length > 0) offers.forEach(function(b){ addBulkRow(b); });
        ...
        row.innerHTML = `
            ...
            <td>
                <select class="bulk-edit-input bulk-category">
                    ...
                    ${categories.map(c => `<option value="${c.id}" ${item && item.service_offer_category_id == c.id ? 'selected' : ''}>${c.name}</option>`).join('')}
                </select>
            </td>
            <td>
            ...
            <td><input type="text" class="bulk-edit-input bulk-link-to" value="${item ? item.link_offer_to || '' : ''}"></td>
    ...
    document.getElementById('bulkEditSaveBtn').onclick = function(){
        ...
        rows.forEach(function(row){
            ...
            data.push({
                ...
                service_offer_category_id: row.querySelector('.bulk-category').value || null,
        ...
        ajax('POST', ROUTES.bulkUpdate, {offers: data}, function(err, res){
            if(err) return alert(err);
            offers = res; renderTable(); bulkOverlay.classList.remove('open'); showToaster('Bulk data saved.');
        });
    };

    // Global Media Integration
    var selectedMediaUrl = null, mediaPage = 1;
    window.openMediaSelector = function(inputId, previewId) {
        window._currentMediaTarget = { input: inputId, preview: previewId };
