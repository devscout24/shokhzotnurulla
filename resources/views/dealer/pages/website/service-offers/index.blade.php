                            ...
                            <div style="padding: 30px; background: #fff;">
                                ...
                                <div style="margin-bottom: 25px;">
                                    ...
                                    <input type="text" id="subtitleInput" class="bulk-input"
                                        placeholder="Price or percentage discount offered">
                                </div>
    ...
    <script>
        (function() {
            ...
            window._trash = function(id) {
                customConfirm('Are you sure you want to delete this offer?', function() {
                    ajax('DELETE', ROUTES.destroy.replace('__ID__', id), null, function(err) {
                        ...
                        offers = offers.filter(r => r.id != id);
                        renderTable();
                        showToaster('Offer deleted.');
                    });
                });
            };
            ...
            window.openForm = function(id) {
                editingId = id;
                var item = id ? offers.find(function(b) {
                    return b.id == id;
                }) : null;
                ...
                categories.forEach(function(c) {
                    catSel.innerHTML += '<option value="' + c.id + '"' + (item && item
                            .service_offer_category_id == c.id ? ' selected' : '') + '>' + c.name +
                        '</option>';
                });
            ...
            document.getElementById('saveBtn').onclick = function() {
                ...
                ajax(editingId ? 'PATCH' : 'POST', editingId ? ROUTES.update.replace('__ID__', editingId) : ROUTES
                    ...
                    function(err, res) {
                        if (err) return alert(err);
                        if (editingId) {
                            var idx = offers.findIndex(function(b) {
                                return b.id == editingId;
                            });
                            if (idx > -1) offers[idx] = res;
                        } else offers.push(res);
                        window.location.hash = '#list';
                        renderTable();
                        showToaster('Offer saved.');
                    });
            };
            ...
            function openBulkEdit() {
                ...
                if (offers.length > 0) offers.forEach(function(b) {
                    addBulkRow(b);
                });
