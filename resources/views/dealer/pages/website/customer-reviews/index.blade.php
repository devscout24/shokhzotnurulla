...
@push('page-styles')
    ...
    <style>
        ...
        #mediaModalOverlay .btn-save-red:disabled {
            background: #f59e9e;
            cursor: not-allowed;
        }

        #mediaModalOverlay .btn-save-red {
            border-radius: 8px;
            padding: 10px 25px;
        }
...
@section('page-content')
                        ...
                        <div class="rc-main-container" style="border: 1px solid #e0e0e0; border-radius: 8px;">
                            ...
                            <div style="padding: 30px; background: #fff;">
                                <div style="margin-bottom: 30px;">
                                    ...
                                    <input type="text" id="reviewerNameInput" class="bulk-input"
                                        placeholder="Jane Smith, Google, AutoTrader">
                                </div>

                                <div style="margin-bottom: 30px;">
                                    <label class="bulk-col-label">Review Headline</label>
                                    <input type="text" id="reviewHeadlineInput" class="bulk-input"
                                        placeholder="Jane Smith, Google, AutoTrader">
                                </div>
                                ...
                                <div style="margin-bottom: 30px;">
                                    ...
                                    <input type="number" id="starCountInput" class="bulk-input" min="1"
                                        max="5" value="5">
                                </div>
                                ...
                                <div style="margin-bottom: 40px;">
                                    ...
                                    <div class="bulk-rte">
                                        ...
                                        <div contenteditable="true" id="contentInput" class="rte-textarea"></div>
                                    </div>
                                </div>
    ...
    <div class="faq-modal-overlay" id="mediaModalOverlay">
        <div class="faq-modal" style="width: 1100px; max-width: 95vw; height: 90vh;">
            ...
            <div
                style="padding: 25px 30px; border-bottom: 1px solid #f9f9f9; display: flex; align-items: center; justify-content: space-between;">
                <div style="font-size: 14px; font-weight: 700; color: #333;">Select image:</div>
                <div style="display: flex; gap: 15px; align-items: center;">
                    <div style="width: 320px;"><input type="text" id="mediaSearchInput" class="bulk-input"
                            placeholder="Search media..."></div>
                    <button class="btn-save-red" style="padding: 10px 22px; font-size: 13px;"><i
                            class="bi bi-plus-lg"></i> Add Media</button>
                </div>
            </div>
            <div
                style="padding: 15px 30px; display: flex; justify-content: flex-end; align-items: center; gap: 10px; border-bottom: 1px solid #f9f9f9;">
                <div
                    style="font-size: 12px; color: #666; background: #f8f9fa; padding: 8px 15px; border: 1px solid #eef0f2; border-radius: 4px 0 0 4px; border-right: none;">
                    Display</div>
                <select class="bulk-select" id="mediaDisplaySelect"
                    style="width: 150px; padding: 7px 10px; font-size: 12px; border-radius: 0 4px 4px 0;">
                    <option value="expanded">Expanded</option>
                    <option value="comfortable">Comfortable</option>
                    <option value="compact">Compact</option>
                </select>
            </div>
            <div class="media-grid expanded" id="mediaGrid" style="padding: 30px; border-bottom: 1px solid #f9f9f9;">
            </div>
            <div style="padding: 20px 30px; background: #fff; display: flex; flex-direction: column; align-items: center;">
                <div style="width: 100%; display: flex; justify-content: flex-end; margin-bottom: 20px;">
                    <button class="btn-save-red" id="mediaSelectBtn" disabled style="padding: 12px 35px;"><i
                            class="bi bi-check-lg"></i> Select Media</button>
                </div>
    ...
    <script>
        (function() {
            ...
            window._trash = function(id) {
                customConfirm('Are you sure you want to delete this review?', function() {
                    ajax('DELETE', ROUTES.destroy.replace('__ID__', id), null, function(err) {
                        ...
                        renderTable();
                        showToaster('Review deleted successfully.');
                    });
                });
            };
            ...
            window.openForm = function(id) {
                editingId = id;
                var item = id ? reviews.find(function(b) {
                    return b.id == id;
                }) : null;
                ...
                document.getElementById('reviewDateInput').value = item && item.review_date ? item.review_date
                    .substring(0, 10) : '';
                ...
                categories.forEach(function(c) {
                    catSel.innerHTML += '<option value="' + c.id + '"' + (item && item
                            .customer_review_category_id == c.id ? ' selected' : '') + '>' + c.name +
                        '</option>';
                });
            ...
            document.getElementById('saveBtn').onclick = function() {
                ...
                ajax(editingId ? 'PATCH' : 'POST', editingId ? ROUTES.update.replace('__ID__', editingId) : ROUTES
                    ...
                    function(err, res) {
                        if (err) return alert(err);
                        if (editingId) {
                            var idx = reviews.findIndex(function(b) {
                                return b.id == editingId;
                            });
                            if (idx > -1) reviews[idx] = res;
                        } else reviews.push(res);
                        window.location.hash = '#list';
                        renderTable();
                        showToaster('Review saved.');
                    });
            };
            ...
            window._trash = function(id) {
                customConfirm('Are you sure you want to delete this review?', function() {
                    ajax('DELETE', ROUTES.destroy.replace('__ID__', id), null, function(err) {
                        if (err) return alert(err);
                        reviews = reviews.filter(function(b) {
                            return b.id != id;
                        });
                        renderTable();
                        showToaster('Review deleted.');
                    });
                });
            };
            ...
            function renderMedia() {
                ...
                mediaGrid.querySelectorAll('.media-item').forEach(function(item) {
                    item.onclick = function() {
                        mediaGrid.querySelectorAll('.media-item').forEach(i => i.classList.remove(
                            'selected'));
            ...
            function renderMediaPagination(res) {
                ...
                var html = '';
                html += '<button class="pag-btn ' + (res.current_page === 1 ? 'disabled' : '') +
                    '" onclick="if(mediaCurrentPage>1) loadMedia(mediaCurrentPage-1)"><i class="bi bi-chevron-left"></i></button>';

                for (var i = 1; i <= res.last_page; i++) {
                    if (i === 1 || i === res.last_page || (i >= res.current_page - 1 && i <= res.current_page + 1)) {
                        html += '<button class="pag-btn ' + (i === res.current_page ? 'active' : '') +
                ...
                html += '<button class="pag-btn ' + (res.current_page === res.last_page ? 'disabled' : '') +
                    '" onclick="if(mediaCurrentPage<mediaTotalPages) loadMedia(mediaCurrentPage+1)"><i class="bi bi-chevron-right"></i></button>';
                pag.innerHTML = html;
            }
            ...
            function openBulkEdit() {
                ...
                if (reviews.length > 0) reviews.forEach(function(b) {
                    addBulkRow(b);
                });
            ...
            document.getElementById('bulkEditSaveBtn').onclick = function() {
                ...
                rows.forEach(function(row) {
                    ...
                    data.push({
                        ...
                        customer_review_category_id: row.querySelector('.bulk-category').value ||
                            null,
