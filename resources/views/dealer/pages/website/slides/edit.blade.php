...
@section('page-content')
    ...
    <div class="of-master-frame">
        <form action="{{ $routes['update'] }}" method="POST" id="page-builder-form" style="display:contents"
            onsubmit="return prepareFormSubmit()">
            @csrf
            @method('PATCH')
            <div class="of-header shadow-sm">
                <h4 class="m-0 fs-5 text-dark">Edit Slide</h4>
            ...
            <div class="input-group border-bottom bg-white" style="height:55px">
                ...
                <input type="text" value="{{ $slide->title }}" class="form-control border-0 px-4 fs-6" name="title"
                    id="page-title" required autocomplete="off" placeholder="Enter slide title...">
                <span class="bg-white input-group-text border-0 text-muted small px-4 border-start" id="top-status-badge">
                    <i class="fa-solid fa-circle me-2"
                        style="font-size:8px;color:{{ $slide->is_active ? '#27ae60' : '#ced4da' }}"></i> Status:
                    {{ $slide->is_active ? 'Published' : 'Draft' }}
                </span>
            </div>
                    ...
                    <div class="sidebar-content-area" id="sidebar-add-content">
                        ...
                        <div id="sidebar-default-content">
                            ...
                            <div class="block-grid">
                                ...
                                <div class="block-item" draggable="true" data-type="tabs"><i
                                        class="fa-solid fa-layer-group"></i><span>Tabs</span></div>
                ...
                <div class="side-panel-body">
                    ...
                    <div class="hs-row"><label>Publish Date</label>
                        <input type="datetime-local" class="hs-input" name="published_at" id="ps-published-at"
                            value="{{ $slide->published_at ? $slide->published_at->format('Y-m-d\TH:i') : '' }}">
                    </div>
                    ...
                    <div class="hs-row"><label>Tags (Comma separated)</label><input class="hs-input" name="tags"
                            id="ps-tags" value="{{ is_array($slide->tags) ? implode(', ', $slide->tags) : '' }}"
                            placeholder="hero, home, promo"></div>
                    <div class="hs-row"><label>Meta Title</label><input class="hs-input" name="meta_title"
                            id="ps-meta-title" value="{{ $slide->meta_title }}" placeholder="Slide title"></div>
                    <div class="hs-row"><label>Meta Description</label><textarea class="hs-input" name="meta_description"
                            id="ps-meta-description" style="min-height:80px">{{ $slide->meta_description }}</textarea></div>
            ...
            {{-- Slide Revisions Panel (Right Off-canvas) --}}
            <div class="side-panel" id="side-panel-page-revisions">
                <div class="side-panel-header">
                    <span class="side-panel-title">Slide Revisions</span>
                    <button type="button" class="btn-close" onclick="toggleSidePanel('page-revisions')"></button>
                </div>
                <div class="side-panel-body">
                    <p class="text-muted text-center py-4">No revisions found.</p>
                </div>
            </div>

            <input type="hidden" name="content" id="page-content-json" value="{{ $slide->content }}">
            <input type="hidden" name="meta_keywords" id="meta-keywords-hidden" value="{{ $slide->meta_keywords }}">
            <input type="hidden" name="is_featured" value="{{ $slide->is_featured ? 1 : 0 }}">
        </form>
    </div>
@endsection
@push('pannel-scripts')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            try {
                var contentData = {!! $slide->content ?: '[]' !!};
                if (window.renderExistingContent) {
                    window.renderExistingContent(contentData);
                }
            } catch (e) {
                console.error("Error loading slide content:", e);
            }
