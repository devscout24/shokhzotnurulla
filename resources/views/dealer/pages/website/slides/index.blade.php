...
@section('page-content')
    <div class="main-content">
        <div class="page-header-flex">
            ...
            <div class="header-actions">
                <div class="search-input-group">
                    ...
                    <input type="text" class="search-input" placeholder="Search slides">
                </div>
                            ...
                            @foreach ($slides as $slide)
                                <tr>
                                    ...
                                    <td data-label="Title">
                                        <div class="d-flex flex-column">
                                            <div class="d-flex align-items-center">
                                                <a href="{{ str_replace('__ID__', $slide->id, $routes['edit']) }}"
                                                    class="title-col m-0">
                                                    {{ $slide->title }}
                                                </a>
                                            ...
                                            <div class="inline-actions">
                                                ...
                                                <button type="button" class="btn-action action-trash"
                                                    onclick="showDeleteModal({{ $slide->id }})">
                                                    <i class="fa-regular fa-trash-can"></i> Trash
                                                </button>
                                                <button type="button" class="btn-action" onclick="duplicateSlide({{ $slide->id }})">
                                    ...
                                    <td data-label="Author">
                                        <span class="date-text">{{ $slide->dealer->name ?? 'Admin' }}</span>
                                    </td>
                                    <td data-label="Status">
                                        @php $isPublished = $slide->is_active && $slide->published_at && $slide->published_at <= now(); @endphp
                                    ...
                                    <td data-label="Meta Desc." class="text-center">
                                        @if($slide->meta_description)
                                            <i class="fas fa-check-circle meta-check"></i>
                                        @else
                                            <span class="text-muted">-</span>
                                        @endif
                                    </td>
                                    <td data-label="Created"><span class="date-text">{{ $slide->created_at->format('M d, Y') }}</span></td>
                                    <td data-label="Last Update"><span class="date-text">{{ $slide->updated_at->format('M d, Y') }}</span></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        @else
            <div class="content-card empty-state">
                ...
                <h3 class="text-dark fw-bold">No slides found</h3>
                <p class="text-muted">You haven't created any slides yet.</p>
    ...
    <script>
        ...
        function showDeleteModal(slideId) {
        ...
        document.getElementById('confirmDeleteBtn').addEventListener('click', function () {
            if (slideIdToDelete) {
                ...
                let url = '{{ route("dealer.website.slides.destroy", ":id") }}';
                form.action = url.replace(':id', slideIdToDelete);
                form.submit();
            }
        });
        ...
        function duplicateSlide(slideId) {
            if (confirm('Are you sure you want to duplicate this slide?')) {
                ...
                let url = '{{ route("dealer.website.slides.duplicate", ":id") }}';
                form.action = url.replace(':id', slideId);
                form.submit();
            }
        }
    </script>
