...
<main class="main-content" id="mainContent" style="padding:0; background: #f0f2f5; min-height: 100vh;">
    <div style="padding: 30px 45px;">
        <div class="page-header" style="margin-bottom: 25px; border: none; background: transparent; padding: 0;">
            <h2 class="view-title" style="font-size: 24px; font-weight: 700; color: #222;">{{ __('Reusable Content: Job') }}</h2>
            <div class="rc-header-actions">
                ...
                <button class="rc-btn-add" type="button" id="addBtn"><i class="bi bi-plus-lg"></i> {{ __('Add New Job') }}</button>
            </div>
        </div>
                ...
                <div id="formView" style="display: none;">
                    ...
                    <div class="rc-main-container" style="border: 1px solid #e0e0e0; border-radius: 8px;">
                        <div style="padding: 20px 30px; border-bottom: 1px solid #f0f0f0; display: flex; justify-content: space-between; align-items: center; background: #fff;">
                            <h3 style="font-size: 15px; font-weight: 700; color: #333; margin: 0;" id="formViewTitle">Add Job</h3>
                        </div>
                        <div style="padding: 30px; background: #fff;">
                            <div style="margin-bottom: 30px;">
                                <label class="bulk-col-label">Job Title <span>*</span></label>
                                <input type="text" id="jobTitleInput" class="bulk-input" placeholder="What is the question?">
                            </div>
...
(function(){
    ...
    window._trash = function(id){
        customConfirm('Are you sure you want to delete this job?', function(){
            ajax('DELETE', ROUTES.destroy.replace('__ID__', id), null, function(err){
                ...
                jobs = jobs.filter(r => r.id != id); renderTable(); showToaster('Job deleted successfully.');
            });
        });
    };
    ...
    window.openForm = function(id){
        editingId = id;
        var item = id ? jobs.find(function(b){ return b.id == id; }) : null;
        ...
        categories.forEach(function(c){ catSel.innerHTML += '<option value="'+c.id+'"'+(item && item.job_post_category_id == c.id ? ' selected' : '')+'>'+c.name+'</option>'; });
    ...
    document.getElementById('saveBtn').onclick = function(){
        ...
        ajax(editingId?'PATCH':'POST', editingId?ROUTES.update.replace('__ID__', editingId):ROUTES.store, p, function(err, res){
            if(err) return alert(err);
            if(editingId){ var idx = jobs.findIndex(function(b){ return b.id == editingId; }); if(idx>-1) jobs[idx] = res; }
            else jobs.push(res);
            window.location.hash = '#list'; renderTable(); showToaster('Job saved.');
        });
    };
    ...
    function openBulkEdit(){
        ...
        if(jobs.length > 0) jobs.forEach(function(b){ addBulkRow(b); });
    ...
    document.getElementById('bulkEditSaveBtn').onclick = function(){
        ...
        ajax('POST', ROUTES.bulkUpdate, {jobs: data}, function(err, res){
            if(err) return alert(err);
            jobs = res; renderTable(); bulkOverlay.classList.remove('open'); showToaster('Bulk data saved.');
        });
    };
