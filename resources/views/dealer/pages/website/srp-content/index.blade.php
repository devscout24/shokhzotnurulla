...
@push('page-styles')
    <style>
        ...
        .bulk-table-header {
            display: grid;
            grid-template-columns: 160px 150px 180px 180px 200px 140px 300px 110px 40px;
            gap: 12px;
            padding: 12px 15px;
            background: #fff;
            border-bottom: 1px solid #eee;
    ...
    <main class="main-content" id="mainContent" style="padding:0; background: #f0f2f5; min-height: 100vh;">
        <div style="padding: 30px 45px;">
            <div class="page-header" style="margin-bottom: 30px; border: none; justify-content: space-between;">
                <h2 class="view-title" style="font-size: 24px; font-weight: 700; color: #222; margin: 0;">
                    {{ __('Reusable Content: SRP Content') }}</h2>
                <div class="rc-header-actions">
                    <button class="rc-btn-outline" type="button" id="bulkEditBtn"><i class="bi bi-pencil-square"></i>
                        {{ __('Bulk Edit') }}</button>
                    <button class="rc-btn-add" type="button" id="addSrpBtn"><i class="bi bi-plus-lg"></i>
                        {{ __('Add New SRP Content') }}</button>
                </div>
            </div>
                ...
                <div style="flex: 1; display: flex; flex-direction: column;">
                    ...
                    <div id="srpFormView" style="display: none;">
                        <div style="margin-bottom: 20px;">
                            <button class="rc-btn-outline" id="srpBackBtn" style="padding: 6px 14px; font-size: 11px;"><i
                                    class="bi bi-chevron-left" style="font-size: 10px;"></i> Go Back</button>
                        </div>
                        <div class="rc-main-container" style="padding: 0;">
                            ...
                            <div style="padding: 25px 30px;">
                                <div class="field-group">
                                    <label class="field-label">Content nickname (ex: Honda content for /honda)
                                        <span>*</span></label>
                                    <input type="text" id="srpNicknameInput" class="field-input"
                                        placeholder="Used Cars, Trucks, and SUVs in Atlanta, GA">
                                </div>
                                ...
                                <div class="field-group">
                                    ...
                                    <div class="field-hint">Must match SRP URL slug exactly, i.e. /cars, /trucks, /suvs
                                    </div>
                                </div>

                                <div class="field-group">
                                    <label class="field-label">Main header of the page - H1 (optional override)</label>
                                    <input type="text" id="srpH1Input" class="field-input"
                                        placeholder="Used Cars, Trucks, and SUVs in Atlanta, GA">
                                ...
                                <div class="field-group">
                                    ...
                                    <input type="text" id="srpMetaTitleInput" class="field-input"
                                        placeholder="Used Cars, Trucks, and SUVs in Atlanta, GA">
                                </div>

                                <div class="field-group">
                                    <label class="field-label">Meta Description (optional override - ideally 160 characters
                                        or less)</label>
                                    <textarea id="srpMetaDescInput" class="field-input field-textarea"
                                        placeholder="Shop used cars, trucks, and SUVs in Atlanta, GA. Find the best deals on used vehicles at our dealership."></textarea>
                                </div>
                                ...
                                <div style="display: flex; justify-content: flex-start;">
                                    <button class="btn-save-red" id="srpSaveBtn"><i class="bi bi-check-lg"></i>
                                        Save</button>
                                </div>
                            </div>
                        </div>
        ...
        <div class="faq-modal">
            <div class="bulk-modal-content">
                <div class="bulk-header-top">
                    <h3 class="faq-modal-title" style="font-size: 14px; font-weight: 600;">Bulk Edit Content: Search
                        Results (SRP)</h3>
...
@push('page-scripts')
    <script>
        (function() {
            ...
            function renderTable() {
                ...
                contents.forEach(function(c) {
                    html += '<tr>' +
                        ...
                        '<a href="#" class="faq-row-btn" onclick="window._srpEdit(' + c.id +
                        ');return false;"><i class="bi bi-pencil-square"></i> Edit</a>' +
                        '<button class="faq-row-btn trash-btn" onclick="window._srpTrash(' + c.id +
                        ...
                        '<td><span style="color:#666; font-size:14px;">' + (c.author || 'Austin Scholl') +
                        '</span></td>' +
                        '<td><div class="faq-status-wrap"><span class="faq-status-dot published"></span><span>' +
                        c.status + '</span></div></td>' +
                        '</tr>';
                });
                tbody.innerHTML = html ||
                    '<tr><td colspan="4" style="text-align:center;padding:50px;">No content found.</td></tr>';
                document.getElementById('srpSummary').textContent = 'Showing ' + contents.length + ' of ' + contents
                    .length + ' pages';
            }
            ...
            document.getElementById('addSrpBtn').addEventListener('click', function() {
                openForm(null);
            });
            ...
            document.getElementById('srpSaveBtn').addEventListener('click', function() {
                ...
                if (editingId) {
                    ajax('PATCH', ROUTES.update.replace('__ID__', editingId), payload, function(err, res) {
                        ...
                        contents[idx] = res;
                        document.getElementById('srpBackBtn').click();
                        renderTable();
            ...
            function addBulkRow(item) {
                ...
                div.innerHTML =
                    ...
                    '<textarea class="bulk-textarea bulk-md" placeholder="Meta Description">' + (item ? (item
