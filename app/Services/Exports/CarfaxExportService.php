...
class CarfaxExportService
{
    ...
    public function bulkCarfaxXmlAll(Request $request)
    {
        $fileName = 'carfax_webgurus_inventory.xml';
        ...
        $callback = function () {
            ...
            Dealer::query()
                ...
                ->with(['locations', 'domains', 'inventoryFees'])
                ->chunk(20, function ($dealers) use ($output, $formatPrice) {
                    foreach ($dealers as $dealer) {
                        ...
                        // Resolve dealer website
                        $dealerWebsite = '';
                        if ($dealer->domains->isNotEmpty()) {
                            $primaryDomain = $dealer->domains->firstWhere('is_primary', true) ?? $dealer->domains->first();
                            $dealerWebsite = $primaryDomain?->domain ?? '';
                        }
                        if (empty($dealerWebsite)) {
                            $dealerWebsite = $dealer->domain ?? $dealer->staging_domain ?? '';
                        ...
                        Vehicle::withoutGlobalScopes()
                            ...
                            ->whereIn('status', ['active'])
                            ->chunk(100, function ($vehicles) use ($output, $dealer, $location, $dealerFeeVal, $dealerWebsite, $formatPrice) {
                                foreach ($vehicles as $vehicle) {
                                    $url = $dealerWebsite ? "https://{$dealerWebsite}/vehicles/{$vehicle->slug}" : '';
                                    ...
                                    $retailPrice  = $vehicle->prices?->msrp > 0 ? $vehicle->prices->msrp : ($vehicle->list_price ?? 0);
                                    $invoicePrice = $vehicle->prices?->dealer_cost ?? 0;
