...
class CarsDotComService
{
    ...
    public function export(Dealer $dealer, Request $request){
        $fileName = 'carscom_' . $dealer->internal_id . '.csv';
        ...
        $callback = function () use ($columns, $dealer, $dealerAddress, $location_status_arr) {
            ...
            Vehicle::withoutGlobalScopes()->where('dealer_id', $dealer->id)
                ...
                ->chunk(100, function ($vehicles) use ($file, $dealer, $dealerAddress, $location_status_arr) {
                    foreach ($vehicles as $vehicle) {
                        ...
                        $row = [
                            ...
                            $vehicle->status ?? '',
                            $location_status_arr[$vehicle->status] ?? '',
                            $vehicle->vin,
    ...
    {
        $fileName = 'webgurus_inventory.csv';
        ...
        $callback = function () use ($columns, $location_status_arr) {
            $file = fopen('php://output', 'w');
            fputcsv($file, $columns);
            ...
            Dealer::query()
                ...
                ->with('locations')
                ->chunk(20, function ($dealers) use ($file, $location_status_arr) {
                            ...
                            ->chunk(100, function ($vehicles) use ($file, $dealer, $dealerAddress, $location_status_arr) {
                                foreach ($vehicles as $vehicle) {
                                    ...
                                    $remainingRow = [
                                        ...
                                        $vehicle->specs?->interior_material ?? '',
                                        $vehicle->specs?->wheelbase ?? '',
                                        $vehicle->doors ?? '',
                                        $vehicle->specs?->displacement ?? '',
                                        $vehicle->specs?->cylinders ?? '',
                                        $vehicle->engine ?? '',
                                        $vehicle->specs?->transmission_standard ?? '',
                                        $vehicle->factoryOptions?->pluck('label')->implode(' | ') ?? '',
                                        $vehicle->factoryOptions?->pluck('option_key')->implode(' | ') ?? '',
                                        $vehicle->primaryPhoto?->updated_at?->toIso8601String() ?? '',
                                        $vehicle->notes?->dealer_notes ?? '',
                                        $vehicle->updated_at?->toIso8601String() ?? '',
                                        $vehicle->original_price ?? '',
                                        $vehicle->prices?->addon_price ?? '',
                                        $vehicle->prices?->internet_price ?? '',
                                        $vehicle->specs?->factory_certified ?? '',
                                        $vehicle->specs?->dealer_certified ?? '',
                                        $vehicle->model_number ?? '',
                                        $vehicle->specs?->chrome_style_id ?? '',
                                        $vehicle->specs?->exterior_color_code ?? '',
                                        $vehicle->specs?->interior_color_code ?? '',
                                        $vehicle->specs?->mpg_city ?? '',
                                        $vehicle->specs?->mpg_highway ?? '',
                                    ];

                                    fputcsv($file, array_merge($row, $remainingRow));
                                }
                            });
                    }
                });
