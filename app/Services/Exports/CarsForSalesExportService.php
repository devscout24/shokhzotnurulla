...
class CarsForSalesExportService
{
    ...
    public function bulkTxt(Request $request)
    {
        $fileName = 'inventory'.'.txt';
        ...
        $callback = function () use ($columns, $condition, $formatRow) {
            $file = fopen('php://output', 'w');
            fwrite($file, $formatRow($columns));
            ...
            Dealer::query()
                ...
                ->chunk(20, function ($dealers) use ($file, $condition, $formatRow) {
                    foreach ($dealers as $dealer) {
                        Vehicle::withoutGlobalScopes()
                            ...
                            ->with([
                                ...
                                'photos', 'notes', 'specs', 'prices', 'factoryOptions',
                            ])
                            ->whereIn('status', ['active'])
                            ->chunk(100, function ($vehicles) use ($file, $dealer, $condition, $formatRow) {
                                foreach ($vehicles as $vehicle) {
