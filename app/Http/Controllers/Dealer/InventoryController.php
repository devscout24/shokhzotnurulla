...
class InventoryController extends Controller
{
    ...
    public function exportInventory(Request $request): StreamedResponse
    {
        ...
        $columns = [
            'stocknumber', 'dealer_id', 'title', 'vin', 'condition', 'certified',
            'year', 'make', 'model', 'modelnumber', 'trim', 'mileage', 'body',
            'fuel', 'engine', 'drivetrainstandard', 'exteriorcolor',
            'exteriorcolorstandard', 'interiorcolor', 'interiorcolorstandard',
            ...
            $query->chunk(500, function ($vehicles) use ($handle) {
                foreach ($vehicles as $v) {
                    ...
                    $row = [
                        ...
                        $v->fuelType->name ?? '',
                        $v->engine,
                        $v->drivetrainType->name ?? '',
                        $v->exteriorColor->name ?? '',
                        $v->exteriorColor->name ?? '',
                        $v->interiorColor->name ?? '',
                        $v->interiorColor->name ?? '',
                        $v->status,
                        $v->location_status ?? '',
                        $v->days_on_lot,
                        $v->dailyStats->sum('views'),
                        $v->location->name ?? '',
                        $v->photos_count,
                        $v->prices->internet_price ?? 0,
                        $v->prices->dealer_cost ?? 0,
                        $v->prices->msrp ?? 0,
                        $v->original_price ?? 0,
                        $v->prices->special_price ?? 0,
                        $v->prices->addon_price ?? 0,
                        $url,
                        $v->dealer->name ?? '',
                        $v->location->street1 ?? '',
                        $v->location->city ?? '',
    ...
    public function exportSoldModelsByMake(Request $request, int $make_id): StreamedResponse
    {
        ...
        $makeName = Make::find($make_id)?->name ?? 'make';
