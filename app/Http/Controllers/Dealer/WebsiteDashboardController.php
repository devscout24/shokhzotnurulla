...
class WebsiteDashboardController extends Controller
{
    ...
    private function getTrafficChannelData($dealerId, $startDate, $endDate)
    {
        ...
        foreach ($logs as $log) {
            ...
            if ($medium === 'organic' || (str_contains($referrer, 'google') || str_contains($referrer, 'bing'))) {
                ...
                $channel = 'Referral';
            } elseif (str_contains($referrer, 'facebook') || str_contains($referrer, 'instagram') || str_contains($referrer, 'twitter')) {
                $channel = 'Social';
            }
    ...
    public function exportWebsiteActivity(Request $request)
    {
        ...
        $callback = function () use ($days, $visits, $leads, $clickToCalls, $inventoryCount, $columns) {
            $file = fopen('php://output', 'w');
            fputcsv($file, $columns);

            foreach ($days as $day) {
                $dayVisits = $visits[$day] ?? null;
                ...
                fputcsv($file, [
                    $day,
                    $dayVisits?->count ?? 0,
                    $dayVisits?->unique_count ?? 0,
                    $dayLeads,
                    $dayLeads, // average_leads (simplified)
                    $dayLeads, // min_leads
                    $dayLeads, // max_leads
