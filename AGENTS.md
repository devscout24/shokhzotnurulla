...
- **Key Features section**: Starred factory options (`$vehicle->factoryOptions` where `pivot->is_starred === true`) — max 9, rendered as pill-style cards in a 3-column grid with an SVG icon per option. Icons live in `public/assets/frontend/img/features/{option_key}.svg`; missing icons hide via `onerror`. The `features` relationship is unused on the VDP view. The `factoryOptions` eager load in `VehicleDetailService::loadVehicle()` includes `option_key` and `withPivot('is_starred')`.
